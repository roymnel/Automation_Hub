# hr_deliver.py
# Advanced delivery script with PDF + email for HR Automation
...
def generate_receipt_pdf(output_path="receipt_hr.pdf"):
    pdf = FPDF()
    pdf.add_page()
    ...
    pdf.cell(200, 10, txt="Automation Hub – HR Service Receipt", ln=True, align='C')

    pdf.set_text_color(0, 0, 0)
    pdf.ln(10)
    pdf.cell(200, 10, txt=f"Date: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", ln=True)
    pdf.cell(200, 10, txt="Service: HR Automation", ln=True)
    pdf.cell(200, 10, txt="Delivery Method: Scripted PDF Receipt", ln=True)
    pdf.cell(200, 10, txt="Price Paid: $189.00", ln=True)

    pdf.output(output_path)
    return output_path

def deliver_service(customer_email):
    print("=== HR SERVICE DELIVERY ===")
    print("[✓] Delivery started...")
    pdf_path = generate_receipt_pdf()

    subject = "Your HR Automation Receipt"
    body = (
        "Hello,\n\n"
        "Thank you for your purchase of the HR Automation service from Automation Hub.\n"
