# legal_deliver.py
# Advanced delivery script with PDF + email for Legal Automation
...
def generate_receipt_pdf(output_path="receipt_legal.pdf"):
    pdf = FPDF()
    pdf.add_page()
    ...
    pdf.cell(200, 10, txt="Automation Hub – Legal Service Receipt", ln=True, align='C')

    pdf.set_text_color(0, 0, 0)
    pdf.ln(10)
    pdf.cell(200, 10, txt=f"Date: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", ln=True)
    pdf.cell(200, 10, txt="Service: Legal Automation", ln=True)
    pdf.cell(200, 10, txt="Delivery Method: Scripted PDF Receipt", ln=True)
    pdf.cell(200, 10, txt="Price Paid: $249.00", ln=True)

    pdf.output(output_path)
    return output_path

def deliver_service(customer_email):
    print("=== LEGAL SERVICE DELIVERY ===")
    print("[✓] Delivery started...")
    pdf_path = generate_receipt_pdf()

    subject = "Your Legal Automation Receipt"
    body = (
        "Hello,\n\n"
        "Thank you for your purchase of the Legal Automation service from Automation Hub.\n"
